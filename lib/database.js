...
// Database and assets paths - try multiple locations
const possibleDbPaths = [
  path.join(process.cwd(), 'public', 'data', 'o_easy_ta.db'),
  path.join(process.cwd(), 'data', 'o_easy_ta.db'),
  path.join(__dirname, '..', 'public', 'data', 'o_easy_ta.db'),
  path.join(__dirname, '..', 'data', 'o_easy_ta.db')
];
...
// Set asset paths based on database location
const dataDir = DB_PATH ? path.dirname(DB_PATH) : path.join(process.cwd(), 'public', 'data');
const STUDENTS_PHOTOS_PATH = path.join(dataDir, 'students');
const BARCODES_PATH = path.join(dataDir, 'barcodes');
const ID_CARDS_PATH = path.join(dataDir, 'id_cards');
...
class Database {
  ...
  init() {
    try {
      console.log('🔍 Searching for database in multiple locations...');
      console.log('📁 Possible paths:', possibleDbPaths);
        ...
        try {
          ...
          if (fs.existsSync(publicDir)) {
            ...
            if (fs.existsSync(dataDir)) {
              console.log('📁 Contents of public/data:', fs.readdirSync(dataDir));
            }
          }
        } catch (e) {
          console.log('❌ Cannot read directories:', e.message);
        }
        return;
      }
